...
_G.collectNoneMutated = function() local n=false while _G.runningCollectNoneMutated do if _G.checkEventStatus() then if not n then _G.notify("Paused: A selected event is active. Waiting...",3,"triangle-alert") n=true end while _G.checkEventStatus() and _G.runningCollectNoneMutated do task.wait(1) end if _G.runningCollectNoneMutated then _G.notify("Resuming auto collection - event ended",2,"check") n=false end end if not _G.runningCollectNoneMutated then break end Farm,ImportantFolder,Plants_Physical,Objects_Physical=_G.GetPlayerFarm(_G.localPlayer.Name) for _,p in ipairs(Plants_Physical:GetChildren()) do if not _G.runningCollectNoneMutated or _G.checkEventStatus() then break end if p:IsA("Model") then local f=p:FindFirstChild("Fruits") if f then for _,x in ipairs(f:GetChildren()) do if not _G.runningCollectNoneMutated or _G.checkEventStatus() then break end if not _G.hasAnyMutation(x,_G.sortedMutations) and not x:GetAttribute("Favorited") then pcall(function() _G.ByteNetReliable:FireServer(_G.harvestBuffer,{x}) _G.notify("Collected "..x.Name,1,"check") end) task.wait() end end elseif not _G.hasAnyMutation(p,_G.sortedMutations) and not p:GetAttribute("Favorited") then pcall(function() _G.ByteNetReliable:FireServer(_G.harvestBuffer,{p}) _G.notify("Collected "..p.Name,1,"check") end) task.wait() end end end task.wait(1) end _G.collectNoneThread=nil end
