...
_G.autoplantSeed=function()local hrp=getHumanoidRootPart(plantEvent,AutoPlantSeed)if not hrp then return end local plantPosition=hrp.Position local char=getCurrentCharacter()while AutoPlantSeed do if char then for _,tool in ipairs(char:GetChildren())do if tool:IsA("Tool")and string.find(tool.Name:lower(),"seed")then local args={Vector3.new(plantPosition.X,plantPosition.Y,plantPosition.Z),tool:GetAttribute("Seed")}pcall(function()plantEvent:FireServer(unpack(args))end)task.wait()end end end task.wait()end end
