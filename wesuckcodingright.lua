...
task.defer( function()
    _G.refreshFruitsList = function(TargetDropdown, TargetPlayer)
        local Plants_Physical

        _, _, Plants_Physical,_ = _G.GetPlayerFarm(TargetPlayer)
...
task.defer(function()  
_G.refreshPlayerList = function(targetDropdown)
    local players = {}
    for _, player in ipairs(game.Players:GetPlayers()) do
        if player ~= localPlayer then table.insert(players, player.Name) end
    end
    table.sort(players)
    targetDropdown:Refresh(players)
end
end)


task.defer(function()
    parseTargetFruits = function(TargetDropdown)
    local uniqueFruits = {}
    ...
    if #parsedList == 0 then
        parsedList = { "No plants found" }
    end
